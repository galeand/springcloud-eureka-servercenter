...
# 远程调用推荐使用springcloud-gateway  
Spring Cloud Gateway ，相比之前我们使用的 Zuul（1.x） 它有哪些优势呢？Zuul（1.x） 基于 Servlet，使用阻塞 API，它不支持任何长连接，如 WebSockets，Spring Cloud Gateway 使用非阻塞 API，支持 WebSockets，支持限流等新特性。  
# gateway的一个问题？
...
但是gateway有一个优点，支持长连接，这一点比zuul强了很多！
